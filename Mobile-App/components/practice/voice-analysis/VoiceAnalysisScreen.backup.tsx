...
export const VoiceAnalysisScreen: React.FC<VoiceAnalysisScreenProps> = ({
  ...
  const handleRecordingComplete = useCallback(async (uri: string, duration: number) => {
    ...
    try {
      ...
      const referenceShloka = selectedShloka.text;
      ...
      const mockResult = {
        overallScore: 85,
        confidence: 92,
        transcription: selectedShloka?.text || 'Unable to transcribe',
        metrics: {
          pronunciation: { score: 84, label: 'Pronunciation Clarity', color: '#4CAF50', icon: 'microphone' },
          syllables: { score: 85, label: 'Syllable Accuracy', color: '#4CAF50', icon: 'format-letter-case' },
          words: { score: 90, label: 'Word Accuracy', color: '#4CAF50', icon: 'text' },
          meter: { score: 88, label: 'Meter Accuracy', color: '#4CAF50', icon: 'metronome' },
        },
        feedback: 'Good attempt! Your pronunciation is accurate. Main areas for improvement: compound word joining and dental consonant clarity.',
        suggestions: 'Focus on connecting compound words smoothly. Practice the dental consonants with proper pronunciation.',
        errors: [],
      };
