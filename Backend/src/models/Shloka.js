...
const shlokaSchema = new mongoose.Schema({
  ...
  metadata: {
    ...
    category: [{
      ...
      required: true,
      enum: [
        'mantra', 'devotional', 'philosophical', 'educational',
        'healing', 'protection', 'prosperity', 'knowledge',
        'peace', 'strength', 'purification', 'meditation'
      ]
    }],
    tags: [{
      type: String,
...
shlokaSchema.pre('save', function(next) {
  if (this.isModified('community')) {
    ...
    const decayFactor = Math.pow(0.99, ageInDays); // Older content gets lower popularity
