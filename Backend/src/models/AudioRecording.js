...
audioRecordingSchema.virtual('improvementPercentage').get(function() {
...
audioRecordingSchema.pre('save', function(next) {
  if (this.isModified('processing.status')) {
    ...
    } else if (['completed', 'failed'].includes(this.processing.status) && !this.processing.completedAt) {
...
audioRecordingSchema.methods.generateDetailedFeedback = function() {
  ...
  if (analysis.overallAccuracy >= 90) {
    ...
    feedback.push('Needs improvement. Consider practicing more slowly and focusing on individual syllables.');
  }
