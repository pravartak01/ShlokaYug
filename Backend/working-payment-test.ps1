# SHLOKAYUG PAYMENT GATEWAY - FINAL COMPREHENSIVE TEST
...
try {
    ...
    Test-Info "Timestamp: $($health.timestamp)"
} catch {
...
try {
    $registerResponse = Invoke-RestMethod -Uri "$BaseUrl/auth/register" -Method POST -ContentType "application/json" -Body ($userData | ConvertTo-Json)
...
Test-Info "Creating order for ₹$($orderData.amount)"
...
try {
    ...
    if ($orderResponse.data.amount -eq $expectedPaisa) {
        Test-Pass "Amount conversion correct (₹ to paisa)"
    } else {
...
$message = "$($TestResults.OrderId)|$($TestResults.PaymentId)"
$hmac = [System.Security.Cryptography.HMACSHA256]::new([System.Text.Encoding]::UTF8.GetBytes($secret))
$hash = $hmac.ComputeHash([System.Text.Encoding]::UTF8.GetBytes($message))
$validSignature = [System.Convert]::ToHexString($hash).ToLower()

$verifyData = @{
    razorpay_order_id = $TestResults.OrderId
    razorpay_payment_id = $TestResults.PaymentId
    razorpay_signature = $validSignature
}

# Test valid signature
try {
    $verifyResponse = Invoke-RestMethod -Uri "$BaseUrl/payments/verify" -Method POST -Headers $headers -Body ($verifyData | ConvertTo-Json)
...
try {
    $invalidResponse = Invoke-RestMethod -Uri "$BaseUrl/payments/verify" -Method POST -Headers $headers -Body ($invalidVerifyData | ConvertTo-Json)
...
try {
    $statusResponse = Invoke-RestMethod -Uri "$BaseUrl/payments/$($TestResults.OrderId)/status" -Headers $headers
    ...
    Test-Info "Amount: ₹$($transaction.amount.total)"
...
try {
    $historyResponse = Invoke-RestMethod -Uri "$BaseUrl/payments/my-payments" -Headers $headers
...
if ($TestResults.Failed -eq 0) {
    ...
    Write-Host "   ✓ HMAC Security Verification" -ForegroundColor Green
    Write-Host "   ✓ Database Transaction Storage" -ForegroundColor Green
...
Write-Host ""
Write-Host "✅ COMPREHENSIVE PAYMENT GATEWAY TEST COMPLETED SUCCESSFULLY!" -ForegroundColor Green
