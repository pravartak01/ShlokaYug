...
import type { User, LoginCredentials, RegisterCredentials, AuthContextType, LearningProgress, UserProfile, GuruRegistrationFormData } from '../types';
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  // Register Guru function
  const registerGuru = async (guruData: GuruRegistrationFormData): Promise<void> => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
      dispatch({ type: 'CLEAR_ERROR' });
      ...
      if (response.success) {
        // Guru application submitted successfully
        dispatch({ type: 'SET_LOADING', payload: false });
      } else {
        throw new Error(response.message || 'Guru registration failed');
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Guru registration failed';
      dispatch({ type: 'SET_ERROR', payload: message });
      throw error;
    }
  };
